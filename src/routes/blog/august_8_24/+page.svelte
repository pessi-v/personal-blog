...
<p>
	I ended up needing an instance with 4GB RAM (2 vCPUs 4GB / 50GB Disk). For Yunohost the image has
	to be Debian 11
</p>
...
<p>
	...
	you’ve set up 2-Factor Authentication (2FA), then go to https://myaccount.google.com/apppasswords
	and generate and save a password for your Rocket.chat instance.
</p>

<p>
	Logged in with your Rocket.chat admin user, first check your admin email address is correctly set
	by navigating to Users on the menu.
</p>
...
<p>
	In your Yunohost admin area you can set up Let’s Encrypt SSL Certificates for your subdomains. Go
	to Domains > your domain > Certificate > Install Let’s Encrypt Certificate.
</p>

<p>
	In my case, there were some warnings, so I opened up the diagnosis page. The diagnosis indicated a
	few warnings and one issue, but since everything seemed to be working in practice, I decided to
	switch on “Ignore diagnosis checks” and proceed with installing the certificates. There seemed to
	be no further problems a and my certificates appear to function like they should! Wrap-up
</p>
